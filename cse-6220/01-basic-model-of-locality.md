...
$y_0$ is the maximum possible value of $R_{\max}$ .
  * ***N.B.*** Take the following to see this more convincingly:

```math
\mathop {\lim }\limits_{I \to \infty } \underbrace {\left\{ {{{{W _ *}} \over W} \cdot \min \left( {1,{I \over B}} \right)} \right\}}_{{R_{\max }}}
```
...
\ \ \ \ \ \ \ \ {{\rm{let\ }}\hat C \equiv b \times b{\rm{\ block\ at\ }}C[i,j]}\\
