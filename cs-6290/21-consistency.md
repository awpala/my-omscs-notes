...
Therefore, in order to prevent violation of sequential consistency in the latter case, it is simply necessary to ***detect*** such intermediate write operations (i.e., `SW`s) accordingly.
  ...
  * Consequently, if such a write operation (i.e., `SW`) is detected (denoted by red in the figure shown above), then any "upstream" read operations (i.e., `LW`s) prior to this point of write operation(s) must be "replayed" (and also fed back to any downstream instructions depending on its value) accordingly.
